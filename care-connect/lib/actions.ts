...
export async function admitPatient(formData: FormData) {
    ...
    try {
        ...
        // Call AdmitPatient Procedure — reserves the room, sets status='Admitted', payment_status='Pending'
        ...
        // ✅ NO invoice or payment created at booking time.
        // The single final bill is generated by DischargePatient() when the patient leaves.
        ...
        return { success: true, roomNumber: result.roomNumber };
...
export async function getDashboardStats() {
    ...
    // 3. Pending & Total Revenue (Subtract Expenses for Net Earning)
    const [revenue] = await pool.query(`
        SELECT 
            SUM(CASE WHEN status = 'Unpaid' THEN net_amount ELSE 0 END) as pending,
            SUM(CASE WHEN status = 'Paid' THEN net_amount ELSE 0 END) - 
            (SELECT COALESCE(SUM(amount), 0) FROM hospital_expenses WHERE category = 'Pharmacy_Restock') as total_paid
        FROM invoices
    `);
