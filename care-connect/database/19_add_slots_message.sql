...
BEGIN
    ...
    IF v_on_leave > 0 THEN
        SET p_message = 'Doctor is not available for the time being';
    ...
    ELSE
        ...
        IF v_start_time IS NOT NULL THEN
            ...
            WHILE TIME(v_current_slot) < v_end_time DO
                ...
                IF v_is_available THEN
                    INSERT INTO temp_slots VALUES (v_slot_time, DATE_FORMAT(v_slot_time, '%l:%i %p'));
                END IF;
        ...
        SELECT slot_time, formatted_time FROM temp_slots;
    END IF;
END //
